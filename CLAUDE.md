...
- **`octopus.py`** — optional Octopus Agile (dynamic tariff) awareness. `fetch_rates()` pulls upcoming half-hourly unit rates from Octopus's public API (no auth) and converts pence→£; no-op (returns None) when `OCTOPUS_PRODUCT_CODE`/`OCTOPUS_REGION` are unset or on error. Served via `GET /api/tariff` (cached 30 min, serves stale on failure); the dashboard's tariff card hides when disabled.
- **`notify.py`** — unified notification dispatch: `notify.send(...)` fans a message out to every channel (`ntfy.send` + `push.send`). All notification call sites in `main.py`/`api.py` use `notify.send`, so new channels need no further wiring. Each channel is independently optional and swallows its own errors.
...
- **`push.py`** — optional VAPID web push (pywebpush). Enabled when both `VAPID_PUBLIC_KEY`/`VAPID_PRIVATE_KEY` are set; browser subscriptions persist to `PUSH_SUBSCRIPTIONS_PATH` (JSON). `send()` fans out in a thread (pywebpush is sync) and prunes subscriptions the push service reports as gone (404/410). Endpoints: `GET /api/push/config`, `POST /api/push/subscribe`, `POST /api/push/unsubscribe`; the SPA's `PushToggle` + service-worker `push` handler complete the loop.
