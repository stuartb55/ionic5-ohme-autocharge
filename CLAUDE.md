...
- **`settings.py`** — runtime-adjustable settings persisted to a JSON file (`SETTINGS_PATH`), all keys in one object so each setter does a read-modify-write. Holds base/weekday/one-time-trip charging settings plus the full `NotificationPreferences` replacement written by `PUT /api/settings/notifications`; loaders validate each persisted field independently and fall back safely. The base target lives on `state.store` (`charge_target` property / `set_charge_target`): the runtime override if set, else `config.CHARGE_TARGET`. `store.effective_target` resolves trip, today's weekday override (in `config.TIMEZONE`), then base; `effective_ready_by` similarly gives trip departure precedence. These are what plug-in handling and snapshots use. When the user hasn't set a permanent ready-by override, the dashboard field auto-populates from Ohme's own configured time. Charging settings go through `api._reapply_target_if_connected` so an active session re-plans immediately. The same file also holds the durable session marker used to avoid duplicate handling across restarts. Persistence is best-effort; failed writes remain in memory for the process lifetime.
