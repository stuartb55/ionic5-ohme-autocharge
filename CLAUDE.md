...
- **Monthly report contract** — `GET /api/reports/monthly` reads exact persisted `daily_stats` and `charge_sessions` units for a DST-safe `[local month start, next month start)` window. JSON is Pydantic-validated; CSV uses explicit account-vs-session column names. Never merge account-wide Ohme daily totals with selected/home-session actuals. Incomplete days, missing measurements, mixed currencies and current-month partial coverage remain visible rather than zero-filled.
- **Data-quality UI** — `Dashboard` polls `/api/data-quality` every five minutes and renders `DataQualitySection` only when persistence is available. Missing actual cost is applicable only when Octopus tariff pricing is configured; unavailable optional integrations must not be presented as failures. Session-related warnings link to the history section.
- **`ntfy.py`** — optional push notifications via ntfy.sh, called directly as `ntfy.send(...)` from the notification sites in `main.py`/`api.py`. Silently disabled when `NTFY_TOPIC` is unset. Call sites first consult the persisted `store.notification_preferences` categories and thresholds; completion persistence/reconciliation remains unconditional even when its message is suppressed. Every notification carries a `title` and an emoji `tags` value (the ntfy `X-Tags` header — e.g. `electric_plug` plug-in, `white_check_mark` finished/reconnected, `warning` problems, `bar_chart` digest — which renders as an icon on all clients, unlike markdown which phones show raw); multi-fact messages use one-fact-per-line bodies. The poll loop also sends a weekly summary digest (`api._maybe_send_weekly_digest` → `_format_digest`) on `WEEKLY_DIGEST_DAY`/`WEEKLY_DIGEST_HOUR` (local), guarded by `store.last_digest_date` so it fires once per scheduled day.
...
- **`settings.py`** — runtime-adjustable settings persisted to a JSON file (`SETTINGS_PATH`), all keys in one object so each setter does a read-modify-write. Holds base/weekday/one-time-trip charging settings, `VehicleProfile` mappings keyed by stable Hyundai vehicle ID, and the full `NotificationPreferences` replacement; loaders validate each persisted field independently and fall back safely. Target precedence is trip → actual vehicle profile → today's weekday override (in `config.TIMEZONE`) → global base; ready-by precedence is trip → vehicle profile → permanent default. `main.handle_plugin_event` resolves with the `vehicle_id` returned by its Bluelink read, while status/reapply use the selected or last-observed ID. Charging settings go through `api._reapply_target_if_connected` so an active session re-plans immediately. The same file also holds the durable session marker used to avoid duplicate handling across restarts. Persistence is best-effort; failed writes remain in memory for the process lifetime.
