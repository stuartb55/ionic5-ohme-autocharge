...
- **`octopus.py`** — optional Octopus awareness. Agile rates are fetched in the background every 30 minutes, cached for projections, and persisted to `tariff_rates` so actual pricing never depends on opening the dashboard. `cost_for_slots` prices the planned schedule with Decimal arithmetic; after completion, `price_energy_buckets` prices reconstructed delivered Wh into integer pence only when every interval is covered exactly once. Gaps, overlaps or telemetry mismatch withhold the actual total and set a quality state. Household consumption discovery/fetching remains separate and feeds the quality-aware house/car attribution in `energy.py`.
...
- **`db.py`** — optional Postgres persistence for charging history (for Grafana). Enabled only when `DATABASE_URL` is set; when unset or the DB is unreachable at startup, every helper is a no-op and the app runs entirely in memory as before (same graceful-degradation pattern as `ntfy`). Alembic migrations in `alembic/` run automatically before the async pool opens. Each physical plug-in receives a persisted UUID `session_key`; `record_session` upserts on that key, lifecycle fields close it on unplug, and `session_events` provides an audit trail. All writes remain best-effort. `main.handle_plugin_event` records a `charge_sessions` row (+ `schedule_snapshots`) per plug-in; `api.poll_loop` appends a `telemetry` row each poll and refreshes `daily_stats` every `DAILY_STATS_INTERVAL`; `api.get_statistics` also upserts `daily_stats` opportunistically. `db.get_soh_history` reads the per-plug-in `soh_percent` column back as a trend (collapsing consecutive unchanged readings to one point per change) for the dashboard's battery-health sparkline via `GET /api/soh-history` (`enabled: false` when persistence is off, mirroring `/api/sessions`). `db.get_all_sessions` reads the full session history (unbounded, chronological) for `GET /api/sessions/export?format=csv|json`, which streams it as a downloadable attachment (404 when persistence is off); the dashboard's recent-sessions card links to both formats. `db.get_session_telemetry(session_id)` returns one session's per-poll points for `GET /api/sessions/{id}/telemetry` — the dashboard expands a session row into a charge curve (`SessionChargeCurve` + the pure `buildChargeCurve` helper). The `grid_consumption` table is upserted by `api._persist_grid_consumption` and read by `GET /api/energy-usage`. See `docs/grafana.md` for the schema and example queries.
